...
//        self.pokemon.append(MapPkm(name: "", dexnum: ""))
    ...
    func AddPokemon()
    {
        ...
        let values = ["id": id, "user": (Auth.auth().currentUser?.uid)!, "lat": lat as Any, "lon": lon as Any, "name": self.pokename, "cp": self.cp, "dexnum": self.dexnum, "sighted": createdAt] as [String : Any]
        doc.setData(values)
        //show alert
        self.show.toggle()
    }
